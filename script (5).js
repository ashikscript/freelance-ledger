...
//
// All data lives in ONE shared Firestore document, so every account you add
// sees and edits the exact same ledger. Don't change this value, or you'll
// lose access to the data already saved.
const DATA_DOC_ID = "0vsGifU3itenxHFi5P6aTVrHS9I1";
...
async function persistAll(){
  ...
  try{
    await setDoc(doc(db, 'users', DATA_DOC_ID), { entries, categories, marketplaces });
  }catch(err){
    console.error(err);
